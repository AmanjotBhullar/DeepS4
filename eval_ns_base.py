...
_TAG = "_seed%d" % _SEED
print("EVAL_MODEL", "Env_Yield_nonspatial_lrloop", "SEED", _SEED, flush=True)
for batch_loop, epoch_loop, lr_loop in [(2, 4, 0.00005)]:
    ...
    def parse_combined_tfr_element(element):
        ...
        zeros = tf.zeros([length_max - height], tf.float32)
        latitude = tf.concat([latitude, zeros], axis=0)
        ...
        zeros = tf.zeros([length_max - height], tf.float32)
        longitude = tf.concat([longitude, zeros], axis=0)
        ...
        return (time_dependent, soil, texture_v, singles, landcover_v, one_hot_encoding, area), cyield, name_feature
    ...
    with strategy.scope():
        ...
        p_estimate = production_estimate_model([
            X_time_district2, X_soil_district2, X_texture_district2, 
        ...
        newmodel = Model(
            [X_time_district, X_soil_district, X_texture_district, X_singles_district, 
             X_landcover_district, input_onehot_encoded_crops, input_pixel_area],
        ...
        best_val_path = './sbatch/Env_Yield_nonspatial_lrloop.pickle'
        ...
        class CustomCallback(keras.callbacks.Callback):
            ...
            def on_epoch_end(self, epoch, logs=None):
                ...
                if val is not None and val < self.best_val:
                    ...
                    self.model.save_weights('./sbatch/Env_Yield_nonspatial_lrloop.weights.h5')
                    # self.best_val = 10
        ...
        class CustomDataGenerator(Sequence):
            ...
            def __getitem__(self, index):
                ...
                inputs = (
                    ...
                    self.X_singles, self.X_landcover, self.input_onehot_encoded_crops, self.area
                )
                targets = self.cyield
        ...
        model_weights_path = './sbatch/Env_Yield_nonspatial_lrloop' + '.weights.h5'
        ...
        # ----------------------------------------------------------------------
        pass
    # ===== Canadian-filtered per-crop MAE (outside strategy.scope) =====
    newmodel.load_weights(model_weights_path)
    import collections as _c
    _crop_names = ['Barley','Canola','Corn','Cotton','Flaxseed','Hay','Lentils','Oats','Peas','Triticale','Rice','Sorghum','Soy','Sugarbeets','SpringWheat','Mustard','WinterWheat','Peanuts','Chickpeas']
    _absc = _c.defaultdict(list)
    _nca = 0
    for _inp, _y, _name in validation_set:
        _in = list(_inp)
        _in[5] = tf.cast(_in[5], tf.float32)
        _p = np.array(newmodel(_in, training=False)).reshape(len(_in[5]), -1).sum(axis=1)
        _yt = np.array(_y).reshape(-1)
        _oh = np.array(_inp[5]); _ci = _oh.reshape(_oh.shape[0], -1).argmax(axis=1)
        _nm = np.array(_name).reshape(-1)
        for _cc, _pp, _tt, _nn in zip(_ci, _p, _yt, _nm):
            _b = _nn if isinstance(_nn, (bytes, bytearray)) else bytes(_nn)
            if b'USA/' in _b:
                continue
            _nca += 1
            _absc[int(_cc)].append(abs(float(_pp) - float(_tt)))
    print('CANADIAN_N', _nca, flush=True)
    for _i in range(19):
        if _absc[_i]:
            print('CROPMAE %s %s n=%d MAE=%.5f' % ("Env_Yield_nonspatial_lrloop", _crop_names[_i], len(_absc[_i]), np.mean(_absc[_i])), flush=True)
    print('EVAL_DONE Env_Yield_nonspatial_lrloop', _SEED, flush=True)
    break
