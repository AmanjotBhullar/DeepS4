...
for batch_loop, epoch_loop, lr_loop in [(2, 4, 0.00005)]:
    ...
    def parse_combined_tfr_element(element):
        ...
        _lat_p = tf.concat([tf.reshape(latitude, [-1]), tf.zeros([length_max - height], tf.float32)], axis=0)
        _lon_p = tf.concat([tf.reshape(longitude, [-1]), tf.zeros([length_max - height], tf.float32)], axis=0)
        return (time_dependent, soil, texture_v, singles, landcover_v, one_hot_encoding, area, spr_embeds), cyield, name_feature, _lat_p, _lon_p, height
    ...
    # ===== Coordinate-substitution sensitivity (Reviewer 3, Comment 3) =====
    # How much do predictions move when a pixel's own coordinates are replaced?
    #   (a) own coordinates                      -> the method used in the paper
    #   (b) nearest OTHER in-bag district centroid
    #   (c) mean over 30 random in-bag coordinates -> the original Monte-Carlo
    newmodel.load_weights(model_weights_path)
    import numpy as _np, collections as _c

    _inbag = _np.load('./sbatch/inbag_latlon.npy')            # (N,2) lat,lon
    _rng = _np.random.default_rng(0)
    _F = 64
    _minr = (2*_np.pi)/360.0
    _maxr = (2*_np.pi)/0.000001
    _lti = _np.log(_maxr/_minr)/(_F-1.0)
    _ts = _minr*_np.exp(_np.arange(_F)*_lti)
    _fm = _np.repeat(_ts[:,None], 2, axis=1)                  # (64,2)

    def _emb(lat, lon):
        """Replicate the trigonometric spatial embedding in numpy."""
        c = _np.stack([lat, lon], axis=-1).reshape(-1,2,1,1)
        c = _np.repeat(_np.repeat(c, _F, axis=2), 2, axis=3)   # (h,2,64,2)
        spr = c*_fm
        out = _np.concatenate([_np.sin(spr[...,0::2]), _np.cos(spr[...,1::2])], axis=-1)
        return out.reshape(len(lat), -1).astype('float32')

    def _pred(inp, cemb):
        # every element must be a tensor -- Keras rejects a mix of tensors and arrays
        z = [tf.convert_to_tensor(x) for x in inp]
        z[5] = tf.cast(z[5], tf.float32)
        z[7] = tf.convert_to_tensor(cemb)
        return float(_np.array(newmodel(z, training=False)).reshape(-1)[0])

    _rows=[]
    import time as _t; _t0=_t.time()
    for _inp,_y,_name,_lat,_lon,_h in validation_set:
        for _b in range(len(_name)):
            _nm=_np.array(_name).reshape(-1)[_b]
            _nb=_nm if isinstance(_nm,(bytes,bytearray)) else bytes(_nm)
            if b'USA/' in _nb: continue
            h=int(_np.array(_h)[_b])
            lat=_np.array(_lat)[_b][:h].astype('float64')
            lon=_np.array(_lon)[_b][:h].astype('float64')
            one=[_np.array(x)[_b:_b+1] for x in _inp]
            base=_np.array(_inp[7])[_b:_b+1].copy()

            # (a) own coordinates
            ca=base.copy(); ca[0,:h,:]=_emb(lat,lon)
            pa=_pred(one, ca)

            # (b) nearest OTHER in-bag district centroid
            clat,clon=lat.mean(),lon.mean()
            d=(_inbag[:,0]-clat)**2+(_inbag[:,1]-clon)**2
            d[d<1e-9]=_np.inf                                   # exclude itself
            j=int(_np.argmin(d))
            cb=base.copy(); cb[0,:h,:]=_emb(_np.full(h,_inbag[j,0]),_np.full(h,_inbag[j,1]))
            pb=_pred(one, cb)

            # (c) mean over 12 random in-bag coordinates (the original procedure used 30)
            acc=[]
            for k in _rng.choice(len(_inbag),12,replace=False):
                cc=base.copy(); cc[0,:h,:]=_emb(_np.full(h,_inbag[k,0]),_np.full(h,_inbag[k,1]))
                acc.append(_pred(one, cc))
            pc=float(_np.mean(acc)); psd=float(_np.std(acc))

            _rows.append((pa,pb,pc,psd))
            print('SENS own=%.5f nn=%.5f rand=%.5f rand_sd=%.5f  [n=%d %.1fs]' % (pa,pb,pc,psd,len(_rows),_t.time()-_t0), flush=True)

    _a=_np.array(_rows)
    print('\nSENS_N %d' % len(_a), flush=True)
    print('SENS_MEAN_ABS_SHIFT_NN     %.5f' % _np.mean(_np.abs(_a[:,1]-_a[:,0])), flush=True)
    print('SENS_MEAN_ABS_SHIFT_RAND %.5f' % _np.mean(_np.abs(_a[:,2]-_a[:,0])), flush=True)
    print('SENS_MEAN_WITHIN_RAND_SD   %.5f' % _np.mean(_a[:,3]), flush=True)
