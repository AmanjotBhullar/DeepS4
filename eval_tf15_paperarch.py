...
import os as _os
_os.chdir('/home/bhullara/projects/def-aali/Thesis_PhD/')
...
# =============================================================================
for batch_loop, epoch_loop, lr_loop in [(1, 1 , 0.001)]:
    ...
    def parse_combined_tfr_element(element):
        ...
        return (time_dependent, soil, texture_v, singles, landcover_v, one_hot_encoding, area, spr_embeds), cyield, name_feature


    def add_mask_and_weights(features, label):
        # Unpack features:
        # (time_dependent, soil, texture_v, singles, landcover_v, one_hot_encoding, area, spr_embeds)
        X_time, X_soil, X_texture, X_singles, X_landcover, one_hot_encoding, area, cemb = features
        
        # Generate a random mask for X_time (with shape matching X_time).
        # (Original probability: 95% keep, 5% zeroed out)
        mask = tf.cast(tf.random.uniform(tf.shape(X_time)) < 0.95, X_time.dtype)
        X_time_masked = X_time * mask
    
        # Compute sample weights (as in your custom generator).
        # weights = tf.constant([
        ...
        # ])
        weights = tf.constant([
            15.0, 40.0, 1.0, 15.0, 50.0, 
            6.0, 50.0, 5.0, 50.0, 0.0, 
            0.0, 20.0, 10.0, 60.0, 10.0, 
            50.0, 7.0, 15.0, 0.0
        ])
        weights = 0.02 * weights * tf.cast(one_hot_encoding, tf.float32)
        sample_weights = tf.reduce_sum(weights, axis=1, keepdims=True)
        
        # Repack features with the masked time-dependent tensor.
        new_features = (X_time_masked, X_soil, X_texture, X_singles, X_landcover, one_hot_encoding, area, cemb)
        return new_features, label, sample_weights
    ...
    def get_dataset(filenames):
        ...
        # Add the mask and sample weights transformation.
        pass  # add_mask_and_weights disabled for evaluation
    ...
    # (override removed) training_names kept as full list
    # (override removed) validation_names kept as full list
    ...
    with strategy.scope():
        ...
        best_val_path = './sbatch/Env_Yield_noA_lrloop_tf15.pickle'
        ...
        class CustomCallback(keras.callbacks.Callback):
            ...
            def on_epoch_end(self, epoch, logs=None):
                ...
                if val is not None and val < self.best_val:
                    ...
                    self.model.save_weights('./sbatch/Env_Yield_noA_lrloop_tf15.weights.h5')
                    self.best_val = 10
        ...
        model_weights_path = './sbatch/Env_Yield_noA_lrloop_tf15.weights.h5'
        if os.path.isfile(model_weights_path):
            newmodel.load_weights(model_weights_path)

    # ===================== Canada-only per-crop evaluation =====================
    print("PARAMS", newmodel.count_params(), flush=True)
    import collections as _c
    _crop=['Barley','Canola','Corn','Cotton','Flaxseed','Hay','Lentils','Oats','Peas','Triticale',
           'Rice','Sorghum','Soy','Sugarbeets','SpringWheat','Mustard','WinterWheat','Peanuts','Chickpeas']
    _abs=_c.defaultdict(list); _n=0
    for _inp,_y,_nm in validation_set:
        ...
        _nn=np.array(_nm).reshape(-1)
        for _c2,_pp,_tt,_name in zip(_ci,_p,_yt,_nn):
            _b=_name if isinstance(_name,(bytes,bytearray)) else bytes(_name)
            if b'USA/' in _b: continue
            _n+=1; _abs[int(_c2)].append(abs(float(_pp)-float(_tt)))
    print('CANADIAN_N',_n,flush=True)
    for _i in range(19):
        if _abs[_i]:
            print('CROPMAE tf15_paperarch %s n=%d MAE=%.5f'%(_crop[_i],len(_abs[_i]),np.mean(_abs[_i])),flush=True)
