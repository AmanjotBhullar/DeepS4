...
_rnd.seed(0); np.random.seed(0); tf.random.set_seed(0)
print("ABLATION_MODEL", "Env_Yield_nonspatial_lrloop", flush=True)
for batch_loop, epoch_loop, lr_loop in [(2, 4, 0.00005)]:
    ...
    with strategy.scope():
        ...
        # ----------------------------------------------------------------------
        pass
    # ===== inference-time ablation on the Canadian hold-out (outside strategy.scope) =====
    newmodel.load_weights(model_weights_path)
    import numpy as _np
    _conds = {'baseline': [], 'climate': [0], 'soil': [1, 2], 'landscape': [3], 'landcover': [4]}
    _err = {k: [] for k in _conds}
    for _inp, _y, _name in validation_set:
        _base = list(_inp); _base[5] = tf.cast(_base[5], tf.float32)
        _yt = _np.array(_y).reshape(-1)
        _nm = _np.array(_name).reshape(-1)
        _keepmask = _np.array([b'USA/' not in (n if isinstance(n, (bytes, bytearray)) else bytes(n)) for n in _nm])
        if not _keepmask.any():
            continue
        for _cond, _idxs in _conds.items():
            _in = list(_base)
            for _gi in _idxs:
                _t = _in[_gi]
                _in[_gi] = tf.ones_like(_t) * tf.reduce_mean(_t)
            _p = _np.array(newmodel(_in, training=False)).reshape(len(_base[5]), -1).sum(axis=1)
            for _k, _pp, _tt in zip(_keepmask, _p, _yt):
                if _k:
                    _err[_cond].append(abs(float(_pp) - float(_tt)))
    _bmae = _np.mean(_err['baseline'])
    print('=== INFERENCE-TIME ABLATION (%s, Canadian test, n=%d) ===' % ("Env_Yield_nonspatial_lrloop", len(_err['baseline'])), flush=True)
    for _cond in _conds:
        _mm = _np.mean(_err[_cond])
        _inc = 100.0 * (_mm - _bmae) / _bmae if _bmae > 0 else 0.0
        print('ABLATE %-10s MAE=%.5f dMAE=%+.5f increase=%+.1f%%' % (_cond, _mm, _mm - _bmae, _inc), flush=True)
    print('ABLATE_DONE Env_Yield_nonspatial_lrloop', flush=True)
    break
