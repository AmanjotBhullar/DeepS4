...
# =============================================================================
_SEED = int(os.environ.get("RUN_SEED", "0"))
random.seed(_SEED); np.random.seed(_SEED); tf.random.set_seed(_SEED)
_TAG = "_seed%d" % _SEED
print("EVAL_MODEL", "Env_Yield_noA_lrloop", "SEED", _SEED, flush=True)
for batch_loop, epoch_loop, lr_loop in [(2, 4, 0.00005)]:
    ...
    def parse_combined_tfr_element(element):
        ...
        _true_h = tf.shape(area)[0]
        _stride = tf.maximum(1, tf.cast(tf.math.ceil(
            tf.cast(_true_h, tf.float32) / float(length_max)), tf.int32))
        _idx = tf.range(0, _true_h, _stride)[:length_max]
        area = tf.gather(area, _idx)
        ...
        time_dependent = tf.gather(tf.io.parse_tensor(raw_time_dependent, out_type=tf.float32)[:, 2:8, :], _idx)
        ...
        soil = tf.gather(tf.io.parse_tensor(raw_soil, out_type=tf.float32), _idx)
        ...
        texture_v = tf.gather(tf.io.parse_tensor(raw_texture_v, out_type=tf.float32), _idx)
        ...
        singles = tf.gather(tf.io.parse_tensor(raw_singles, out_type=tf.float32), _idx)
        ...
        landcover_v = tf.gather(tf.io.parse_tensor(raw_landcover_v, out_type=tf.float32), _idx)
        ...
        latitude = tf.gather(tf.io.parse_tensor(raw_latitude, out_type=tf.float32), _idx)

        # Process and pad the 'longitude' tensor.
        raw_longitude = content['longitude']
        longitude = tf.gather(tf.io.parse_tensor(raw_longitude, out_type=tf.float32), _idx)

        # --- per-pixel geolocation for map building (padded to length_max) ---
        _pad0 = tf.zeros([length_max - height], tf.float32)
        _lat_pad = tf.concat([latitude, _pad0], axis=0)
        _lon_pad = tf.concat([longitude, _pad0], axis=0)
        ...
        return (time_dependent, soil, texture_v, singles, landcover_v, one_hot_encoding, area, spr_embeds), cyield, name_feature, _lat_pad, _lon_pad, height
    ...
    with strategy.scope():
        ...
        # ----------------------------------------------------------------------
        pass
    # ===== Per-pixel suitability over Canadian districts (for revised Figure 3) =====
    # `length_max` was reset to None before the model was built (dynamic input shapes).
    # The parse function pads to it, so restore the training value before building datasets --
    # this is exactly the value the original eval used when it constructed validation_set.
    length_max = 40000
    newmodel.load_weights(model_weights_path)

    # p_estimate BEFORE area-weighting == per-pixel, per-crop suitability.
    # It is crop-agnostic: the one-hot input only gates the final district sum.
    pixel_model = Model(newmodel.inputs, newmodel.get_layer('reshape_district').output)

    CROPS19 = ['Barley','Canola','Corn','Cotton','Flaxseed','Hay','Lentils','Oats','Peas',
               'Triticale','Rice','Sorghum','Soy','Sugarbeets','SpringWheat','Mustard',
               'WinterWheat','Peanuts','Chickpeas']
    WANT = ['Barley','Canola','Corn','Flaxseed','Lentils','Oats','Peas','Soy','SpringWheat','WinterWheat']
    WIDX = [CROPS19.index(c) for c in WANT]

    LAT0, LAT1, LON0, LON1, RES = 41.0, 62.0, -141.0, -52.0, 0.025
    NR = int(round((LAT1 - LAT0) / RES)); NC = int(round((LON1 - LON0) / RES))
    sums = np.zeros((len(WANT), NR, NC), dtype=np.float64)
    cnts = np.zeros((NR, NC), dtype=np.int64)
    print('GRID', NR, 'x', NC, 'res', RES, flush=True)

    # Cheap pre-filter: deserialize ONLY 'name' so the expensive full parse
    # runs on Canadian records only (a few hundred, not ~44k).
    def _is_canada(element):
        c = tf.io.parse_single_example(element, {'name': tf.io.FixedLenFeature([], tf.string)})
        nm = tf.reshape(tf.io.parse_tensor(c['name'], out_type=tf.string), [-1])[0]
        return tf.logical_not(tf.strings.regex_full_match(nm, ".*USA/.*"))

    nrec = 0
    npix = 0
    for _flist in (training_names, validation_names):
        _ds = tf.data.TFRecordDataset(_flist, num_parallel_reads=tf.data.AUTOTUNE)
        _ds = _ds.filter(_is_canada)
        _ds = _ds.map(parse_combined_tfr_element, num_parallel_calls=tf.data.AUTOTUNE).batch(1)
        for _inp, _y, _name, _lat, _lon, _h in _ds:
            _in = list(_inp); _in[5] = tf.cast(_in[5], tf.float32)
            pp = np.array(pixel_model(_in, training=False))[0]      # (length_max, 19)
            h  = int(np.array(_h).reshape(-1)[0])
            la = np.array(_lat).reshape(-1)[:h].astype(np.float64)
            lo = np.array(_lon).reshape(-1)[:h].astype(np.float64)
            vals = pp[:h][:, WIDX].astype(np.float64)               # (h, 10)

            r = np.floor((la - LAT0) / RES).astype(np.int64)
            c = np.floor((lo - LON0) / RES).astype(np.int64)
            ok = ((r >= 0) & (r < NR) & (c >= 0) & (c < NC) &
                  np.isfinite(la) & np.isfinite(lo) & np.isfinite(vals).all(axis=1))
            r, c, vals = r[ok], c[ok], vals[ok]
            if r.size:
                np.add.at(cnts, (r, c), 1)
                for k in range(len(WANT)):
                    np.add.at(sums[k], (r, c), vals[:, k])
                npix += int(r.size)
            nrec += 1
            if nrec % 25 == 0:
                print('RECORDS', nrec, 'PIXELS', npix, flush=True)

    print('CANADIAN_RECORDS', nrec, 'PIXELS_BINNED', npix, 'CELLS', int((cnts > 0).sum()), flush=True)
    mean = np.where(cnts > 0, sums / np.maximum(cnts, 1), np.nan).astype(np.float32)
    outp = os.environ.get('NPZ_OUT', '/home/bhullara/projects/def-aali/Thesis_PhD/sbatch/pixel_maps_canada.npz')
    np.savez_compressed(outp, mean=mean, counts=cnts.astype(np.int32),
                        crops=np.array(WANT), bbox=np.array([LAT0, LAT1, LON0, LON1]), res=RES)
    print('SAVED', outp, flush=True)
    print('PIXELMAPS_DONE', flush=True)
    break
