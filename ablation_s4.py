...
print("ABLATION_MODEL", "Env_Yield_noA_lrloop", flush=True)
for batch_loop, epoch_loop, lr_loop in [(2, 4, 0.00005)]:
    ...
    newmodel.load_weights(model_weights_path)
    import numpy as _np
    _conds = {'baseline': [], 'climate': [0], 'soil': [1, 2], 'landscape': [3], 'landcover': [4], 'spatial': [7]}
    ...
    print('=== INFERENCE-TIME ABLATION (%s, Canadian test, n=%d) ===' % ("Env_Yield_noA_lrloop", len(_err['baseline'])), flush=True)
    for _cond in _conds:
    ...
    print('ABLATE_DONE Env_Yield_noA_lrloop', flush=True)
    break
